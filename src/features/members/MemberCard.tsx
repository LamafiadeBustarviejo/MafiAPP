  ...
  return (
    ...
    >
      <div className="p-3 flex flex-col gap-3">
        ...
        {/* Mobile secondary info - Buttons Row */}
        {member.profile?.phone && (
          <div className="md:hidden flex gap-2 w-full">
            <a href={`tel:${member.profile.phone.replace(/\s+/g, '')}`} onClick={e => e.stopPropagation()} className="flex-1 flex items-center justify-center gap-2 bg-zinc-800 hover:bg-zinc-700 py-2 rounded-lg transition-colors text-zinc-300 text-xs font-medium">
              <Phone className="h-3.5 w-3.5" />
              Llamar
            </a>
            <a href={`https://wa.me/34${member.profile.phone.replace(/\s+/g, '')}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="flex-1 flex items-center justify-center gap-2 bg-[#128C7E]/20 hover:bg-[#128C7E]/40 text-[#25D366] py-2 rounded-lg transition-colors text-xs font-medium">
